...
func (repo *SqlUserRepository) AddRoom(id string, roomId string) error {
	_, err := repo.Db.Exec("INSERT INTO room_user (room, user) VALUES (?, ?)", roomId, id)
	return err
}

func (repo *SqlUserRepository) RemoveRoom(id string, roomId string) error {
	_, err := repo.Db.Exec("DELETE FROM room_user WHERE room = ? AND user = ?", roomId, id)
	return err
}
