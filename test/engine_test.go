...
func Test_JoinRoom(t *testing.T) {
	...
	t.Run("Join non-existing room", func(t *testing.T) {
		if err := engine.JoinRoom("nonExistingRoom", testUser1.Id); !errors.Is(err, chat.ErrRoomNotFound) {
			t.Errorf("Expected error %v, got %v", chat.ErrRoomNotFound, err)
		}
	})
	// Test case for joining a room with a non-existing user
	t.Run("Join room with non-existing user", func(t *testing.T) {
		if err := engine.JoinRoom(testRoom1.Id, "nonExistingUser"); !errors.Is(err, chat.ErrUserNotFound) {
			t.Errorf("Expected error %v, got %v", chat.ErrUserNotFound, err)
		}
	})
}
