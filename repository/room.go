...
func (repo *SqlRoomRepository) Add(room chat.Room) error {
...
func (repo *SqlRoomRepository) GetById(id string) (chat.Room, error) {
	var room chat.Room
	err := repo.Db.QueryRow("SELECT id, driver, name FROM rooms WHERE id = ?", id).Scan(&room.Id, &room.Driver, &room.Name)
	return room, err
}

func (repo *SqlRoomRepository) GetAll() ([]chat.Room, error) {
	rows, err := repo.Db.Query("SELECT id, driver, name FROM rooms")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	rooms := make([]chat.Room, 0, 10)
	for rows.Next() {
		var room chat.Room = chat.Room{
